...
class ArticleController extends SysadminController {
   public function index() {
	...
	public function add($id='') {
		...
		$where['is_show']='1';
		//$where['url']= array('eq',"");
		//$where['method']='article';
		//$where['is_menu']='0';
		$Froms=D("Weixin_menu");
		$list=$Froms->where($where)->order("order1 asc")->select();
		foreach($list as $key => $val){
			$wheress['aid']=$val['id'];
			//$wheress['is_diymenu']='1';
			$wheress['is_show']='1';
			$wheress['method']='article';
			$wheress['url']= array('eq',"");
			//$wheress['is_menu']='0';
			$list[$key]['voo']=$Froms->where($wheress)->order("order1 asc")->select();
