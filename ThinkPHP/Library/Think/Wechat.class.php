...
class Wechat
{
    ...
    public function __construct($token, $debug = false)
    {
        $this->token = $token;
        $this->debug = $debug;
		$this->request = (array)simplexml_load_string($GLOBALS["HTTP_RAW_POST_DATA"], 'SimpleXMLElement', LIBXML_NOCDATA);
    }
...
function xiaojo($key,$from,$to) //小九接口函数，该函数可通用于其他程序
{
	...
	$from=urlencode($from);
	$to=urlencode($to);
    $post="chat=".$key."&db=yunmei&pw=123456&from=".$from."&to=".$to;
